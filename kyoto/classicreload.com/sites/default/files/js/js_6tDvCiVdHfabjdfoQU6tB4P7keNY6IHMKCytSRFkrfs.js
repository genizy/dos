...
;(function () {
  var stickyBottomNotGameBanner = function () {
    ...
    var bannerContent = document.querySelector('#underContent .view-header #div-gpt-ad');

    if (!bannerContent) return;

    var bannerTop = banner.getBoundingClientRect().top;
    var bannerContentHeight = bannerContent.getBoundingClientRect().height + 'px';

    if (bannerTop < 0) {
      bannerContent.classList.add('sticky');
      banner.style.height = bannerContentHeight;
    }
    else {
      bannerContent.classList.remove('sticky');
      banner.style.height = '';
    }
  };

  document.addEventListener('DOMContentLoaded', stickyBottomNotGameBanner);
  document.addEventListener('scroll', stickyBottomNotGameBanner);
})();
;
