...
(function (Promise) {
   ...
   function IALoader(canvas, game, callbacks, scale) {
     ...
     if (/archive\.org$/.test(document.location.hostname)) {
       images = { ia: img(window.base+"kyoto/vfat.classicreload.com/v6-legacy/img/PDGO-LOAD.gif"),
                      mame: img(window.base+"kyoto/vfat.classicreload.com/v6-legacy/img/mame.png"),
                      mess: img(window.base+"kyoto/vfat.classicreload.com/v6-legacy/img/mame.png"),
                      dosbox: img(window.base+"kyoto/vfat.classicreload.com/v6-legacy/img/dosbox.png"),
                      sae: img(window.base+"kyoto/vfat.classicreload.com/v6-legacy/img/sae.png"),
                  pce: img(window.base+"kyoto/vfat.classicreload.com/v6-legacy/img/pce.png")
                };
     } else {
       images = { ia: img(window.base+"kyoto/vfat.classicreload.com/v6-legacy/img/PDGO-LOAD.gif"),
                  mame: img(window.base+"kyoto/vfat.classicreload.com/v6-legacy/img/mame.png"),
     ...
     function loadFiles(fetch_file, splash) {
       ...
       return new Promise(function (resolve, reject) {
                            ...
                            loading.then(function (data) {
                                         ...
                                         function () {
                                           splash.setTitle("Failed to download GAME item metadata!");
     ...
     function locateAdditionalJS(filename) {
       if ("file_locations" in modulecfg && filename in modulecfg.file_locations) {
         return get_js_url("dosbox.html.mem");
       }
       return get_js_url("dosbox.html.mem");
       ...
       return fetch(url).then(function(response) {
         ...
         function pump() {
           return reader.read().then(function(result) {
             ...
             cdromLoadedBytes += result.value.length;
             chunks.push(result.value);
             if (loadingTextEl) {
               var mbDone = (cdromLoadedBytes / (1024 * 1024)).toFixed(2);
               loadingTextEl.textContent = 'LOADING... ' + mbDone + ' MB / ' + CDROM_TOTAL_MB + ' MB';
             }
             return pump();
           });
         }
     ...
     function mergeFileParts(baseUrl, numParts, cacheKey) {
       return caches.open('kyoto-cache').then(function(cache) {
         return cache.match(cacheKey).then(function(cached) {
           if (cached) {
             if (loadingTextEl) loadingTextEl.textContent = 'LOADING... (cached)';
             return cached.arrayBuffer();
           }
           cdromLoadedBytes = 0;
           if (loadingTextEl) loadingTextEl.style.display = 'block';
           var parts = [];
           for (var pi = 1; pi <= numParts; pi++) {
             parts.push(fetchWithProgress(baseUrl + '.part' + pi));
           }
           return Promise.all(parts).then(function(buffers) {
             return new Blob(buffers).arrayBuffer().then(function(buffer) {
               cache.put(cacheKey, new Response(new Blob([buffer])));
     ...
     function get_dosbox_files(cfgr, emulator, modulecfg, filelist) {
       ...
       for (i = 0; i < len; i++) {
         ...
         if (filename.endsWith('CDROM.zip')) {
           (function(d, t, u) {
             files.push(mergeFileParts(u, CDROM_PARTS, 'CDROM.zip').then(function(data) {
               return cfgr.mountZip(d, cfgr.localFile(t, data));
             }));
           })(drive, title, url);
         } else {
     ...
     var get_emulator_config_url = function (module) {
       return window.base+'kyoto/vfat.classicreload.com/v6-legacy/' + module + '.json';
     };

     var get_other_emulator_config_url = function (module) {
       return window.base+'kyoto/vfat.classicreload.com/emularity_config_v1/' + module + '.cfg';
     };

     var get_meta_url = function (game_path) {
       var path = game_path.split('/');
       return window.base+"kyoto/vfat.classicreload.com/"+ path[0] +"/"+ path[0] +"_meta.xml";
     };

     var get_files_url = function (game_path) {
       var path = game_path.split('/');
       return window.base+"kyoto/vfat.classicreload.com/"+ path[0] +"/"+ path[0] +"_files.xml";
     };

     var get_zip_url = function (game_path, item_path) {
       if (item_path) {
         return window.base+"kyoto/vfat.classicreload.com/"+ item_path +"/"+ game_path;
       }
       return window.base+"kyoto/vfat.classicreload.com/"+ game_path;
     };

     var get_js_url = function (js_filename) {
       return window.base+"kyoto/vfat.classicreload.com/v6-legacy/"+ js_filename;
     };

     var get_bios_url = function (bios_filename) {
       return window.base+"kyoto/vfat.classicreload.com/emularity_bios_v1/"+ bios_filename;
     };
   ...
   function PCELoader() {
     var config = Array.prototype.reduce.call(arguments, extend);
     config.emulator_arguments = ["-c", "/dosprogram/pce-"+ config.pceModel +".cfg"];
     return config;
   }
   ...
   var build_mame_arguments = function (muted, driver, native_resolution, sample_rate, peripheral, extra_args) {
     ...
     if (peripheral) {
       for (var p in peripheral) {
         if (Object.prototype.propertyIsEnumerable.call(peripheral, p)) {
           args.push('-' + p,
                     '/dosprogram/'+ (peripheral[p][0].replace(/\//g,'_')));
         }
       }
     }
    ...
    var build_dosbox_arguments = function (emulator_start, files) {
      var args = ['-conf', '/dosprogram/ClassicReload.conf'];

      var len = files.length;
      for (var i = 0; i < len; i++) {
        if ('mountpoint' in files[i]) {
          args.push('-c', 'mount '+ files[i].drive +' /dosprogram'+ files[i].mountpoint);
        }
      }
     ...
     var start = function (options) {
       ...
       loading.then(function (_game_data) {
              ...
              .then(function () {
                      ...
                      if ("WebAssembly" in window && game_data.emulatorWASM) {
                        return attach_wasm(game_data.emulatorWASM);
                      } else if (game_data.emulatorJS) {
  ...
  splash.setTitle("Failed to launch emulator.");
     ...
     function attach_wasm(wasm_url) {
       if (wasm_url) {
         return new Promise(function (resolve, reject) {
                              var p = fetch_file("WASM Emulator", wasm_url, 'arraybuffer', false);
                              p.then(function (bytes) {
                                       WebAssembly.compile(bytes)
                                                  .then(function (module) {
                                                          var i = new WebAssembly.Instance(module, { imports: {} });
                                                          Object.keys(i.exports)
                                                                .forEach(function (k) {
                                                                           window[k] = i.exports[k];
                                                                         });
                                                          resolve();
                                                        },
                                                        function () {
